				...
				<?php  if(!empty($inventario)):?>
					<?php foreach ($inventario as $inventarios):?>
						<tr>
							...
							<td><?php echo $inventarios->codigoBarra; ?></td>
							...
							<?php endif ?>
							<td><span class="text-danger"><?php echo $inventarios->fechaVencimiento;?></span></td>
							<?php $dataproducto = $inventarios->idSucursal."*".$inventarios->idProducto."*".$inventarios->producto."*".$inventarios->idLote."*".$inventarios->presentacion."*".$inventarios->existencias."*".round($inventarios->precioCIVA,2)."*".$inventarios->costoExistencias."*".$inventarios->idPresentacion."*".$inventarios->idInventario; ?>
							<td>
								<?php if ($inventarios->existencias > 0): ?>
									<button type="button" id="btn-proTras" class="btn btn-success btn-check3 btn-sm" value="<?php echo $dataproducto?>"><span class="fa fa-check"></span></button>
								<?php endif ?>
							</td>
						</tr>
					<?php endforeach; ?>
